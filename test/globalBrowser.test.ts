...
await describe('puppeteer-launch', async () => {
  await it('Launches a shared default browser', async () => {
    ...
    try {
      ...
      assert.fail(
        'Expected to get a browser instance without waiting, but it failed.'
      )
    }
    ...
    try {
      ...
      assert.fail(
        'Expected an error to be thrown when trying to get a second browser instance without waiting.'
      )
    } catch (error) {
    ...
    try {
      ...
      assert.fail(
        'Expected to get a returned browser instance without waiting, but it failed.'
      )
    }
    ...
    try {
      ...
      assert.fail(
        'Expected to get a new browser instance after idle timeout, but it failed.'
      )
    }
